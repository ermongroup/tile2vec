...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=4,
        ...
        self.linear = nn.Linear(self.z_dim*block.expansion, num_classes)
    ...
    def cosine_loss(self, z_p, z_n, z_d):
        l_n = - nn.CosineSimilarity(dim=1, eps=1e-6)(z_p, z_n)
        l_d = nn.CosineSimilarity(dim=1, eps=1e-6)(z_p, z_d)
        l_n = torch.mean(l_n)
        l_d = torch.mean(l_d)
        loss = l_n + l_d
        l_nd = loss
...
def ResNet18(num_classes=10, in_channels=4, z_dim=512, supervised=False,
    no_relu=False, loss_type='triplet', activation='relu'):
    return ResNet(BasicBlock, [2,2,2,2,2], num_classes=num_classes,
        in_channels=in_channels, z_dim=z_dim, supervised=supervised,
        no_relu=no_relu, loss_type=loss_type, activation=activation)
